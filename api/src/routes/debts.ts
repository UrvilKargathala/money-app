...
debts.post("/", requireAuth, async (c) => {
  ...
  const debtEnt = await getEntitlement(user.user_id, "debts");
  if (!debtEnt.allowed) return c.json({ error: "plan_limit", feature: "debts", plan: debtEnt.plan }, 403);
...
debts.get("/:id/report", requireAuth, async (c) => {
  const { buildReportPdf } = await import("../utils/pdf");
  const user = c.get("user");
  const debtId = c.req.param("id");
  const debt = await getDebtById(user.user_id, debtId);
  if (!debt) return c.json({ error: "Not found" }, 404);
  const schedule = await getScheduleRows(user.user_id, debtId);
  const payments = await getPayments(user.user_id, debtId);
  const pdf = await buildReportPdf({
    title: "Debt Prepayment Comparison",
    subtitle: debt.name,
    sections: [
      { heading: "Debt Summary", columns: ["Metric", "Value"], rows: [
        ["Principal outstanding", debt.principal_outstanding.toFixed(2)],
        ["Interest rate", `${debt.interest_rate}%`],
        ["Monthly EMI", String(debt.emi_amount ?? "-")],
        ["Months remaining", String(debt.months_remaining ?? "-")],
      ]},
      { heading: "Recent Payments", columns: ["Date", "Amount", "Principal", "Interest"], rows:
        payments.slice(0, 12).map((p) => [p.date, p.amount.toFixed(2), p.principal_part.toFixed(2), p.interest_part.toFixed(2)]) },
    ],
    footer: "Generated by MoneyMind.",
  });
  return new Response(new Uint8Array(pdf), {
    headers: { "content-type": "application/pdf",
      "content-disposition": `attachment; filename="debt-${debtId.slice(0,8)}.pdf"` },
  });
});
