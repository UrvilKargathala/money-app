...
import { buildLineChartSvg } from "../utils/svg-chart";
...
/** Trend chart as SVG (contract deviation from PNG — see routes.md R7 note). */
netWorth.get("/chart", requireAuth, async (c) => {
  const user = c.get("user");
  const from = rangeToDate(c.req.query("range"), "1Y");
  const snapshots = await listSnapshots(user.user_id, from, null);
  const svg = buildLineChartSvg({
    title: "Net worth trend",
    points: snapshots.map((s) => ({ date: s.date, value: s.net_worth })),
  });
...
netWorth.get("/report", requireAuth, async (c) => {
  const user = c.get("user");
  const from = rangeToDate(c.req.query("range"), "1Y");
  const [snapshots, milestones] = [
    await listSnapshots(user.user_id, from, null),
    await listMilestones(user.user_id),
  ];

  const pdf = await buildReportPdf({
    title: "Net Worth Report",
    subtitle:
      from === null ? "All time" : `Period ${from} → ${isoDate(new Date())}`,
    footer: "Generated by MoneyMind — snapshot series and milestones.",
    sections: [
      {
        heading: "Snapshot series",
        columns: ["Date", "Assets", "Liabilities", "Net worth"],
        rows: snapshots.slice(-48).map((s) => [
          s.date,
  ...
  return new Response(new Uint8Array(pdf), {
    headers: {
      ...
      "content-disposition": `attachment; filename="net-worth-report-${new Date()
        .toISOString()
        .slice(0, 10)}.pdf"`,
    },
  });
...
netWorth.get("/export", requireAuth, async (c) => {  const user = c.get("user");
  const from = rangeToDate(c.req.query("range"), "All");
